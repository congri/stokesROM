...
mkdir -p "${JOBDIR}"
cp -r "${PROJECTDIR}/featureFunctions/nonOverlappingPolydisperseSpheres" "$JOBDIR"
...
cp "${PROJECTDIR}/ModelParams.m" "${JOBDIR}/ModelParams.m"
cp "${PROJECTDIR}/predictionScript.m" "${JOBDIR}/predictionScript.m"
cp "${PROJECTDIR}/StokesData.m" "${JOBDIR}/StokesData.m"
cp "${PROJECTDIR}/StokesROM.m" "${JOBDIR}/StokesROM.m"
cp "${PROJECTDIR}/trainModel.m" "${JOBDIR}/trainModel.m"
...
echo "/home/programs/matlab/bin/matlab -nodesktop -nodisplay -nosplash -r \"trainModel.m ; quit;\"" >> ./job_file.sh
